            ...
            <main className="flex-1 overflow-y-auto flex flex-col relative w-full">
                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 md:py-8 w-full pb-24 md:pb-8">
                    <div className="mb-6 md:mb-8 text-center md:text-left">
                        ...
                        <p className="text-sm md:text-base text-gray-600 dark:text-gray-400">Real-time market rates from across the region.</p>
                    </div>
                    ...
                    <div className="bg-card-light dark:bg-surface-dark p-4 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 mb-6 flex flex-col md:flex-row gap-4 items-center">
                        ...
                        <button className="w-full md:w-auto px-6 py-2 bg-primary text-white font-medium rounded-lg hover:bg-green-700 transition shadow-sm text-sm md:text-base">
                            Filters
                        </button>
                    </div>
                        ...
                        {filteredPrices.map((item, index) => (
                            <div key={index} className="bg-card-light dark:bg-surface-dark rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 hover:shadow-md transition p-4 md:p-5 group">
                                <div className="flex justify-between items-start mb-4">
                                    <div>
                                        <h3 className="font-bold text-lg md:text-xl text-gray-900 dark:text-white">{item.crop}</h3>
                                        <p className="text-xs md:text-sm text-gray-500 dark:text-gray-400">{item.variety}</p>
                                    </div>
                                    <div className={`text-[10px] md:text-sm font-bold px-2 py-1 rounded-md ${item.change.startsWith('+') ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400' : 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400'}`}>
                                        {item.change}
                                    </div>
                                </div>
                                <div className="mb-4">
                                    <span className="text-2xl md:text-3xl font-bold text-gray-900 dark:text-white">{item.price}</span>
                                </div>
