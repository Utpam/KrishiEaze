                        ...
                        <div className="lg:grid lg:grid-cols-12 lg:gap-16 items-center">
                            <div className="lg:col-span-6 text-center lg:text-left mb-12 lg:mb-0">
                                <div className="inline-flex items-center px-4 py-2 rounded-full bg-green-100 dark:bg-green-900/60 text-primary dark:text-green-300 text-sm font-semibold mb-6 ring-1 ring-inset ring-green-600/20 dark:ring-green-500/30">
                                    <span className="material-icons text-base mr-2">eco</span> {t('landing.empowering')}
                                </div>
                                <h1 className="text-5xl md:text-6xl font-extrabold tracking-tight mb-6 text-gray-900 dark:text-white leading-tight">
                                    {t('landing.hero_title')}<br />
                                    <span className="text-primary dark:text-green-400">{t('landing.hero_title_highlight')}</span>
                                </h1>
                                ...
                                <div className="flex flex-col sm:flex-row gap-4 justify-center lg:justify-start">
                                    ...
                                    <button className="inline-flex items-center justify-center px-8 py-4 text-lg font-bold rounded-full text-primary bg-white border-2 border-primary/20 hover:border-primary hover:bg-green-50 dark:bg-transparent dark:text-white dark:border-gray-600 dark:hover:border-green-400 dark:hover:text-green-400 transition-all">
                                        <span className="material-icons mr-2">play_circle_filled</span> {t('landing.watch_demo')}
                                    </button>
                                </div>
                                <div className="mt-12 grid grid-cols-3 gap-6 border-t border-gray-200 dark:border-gray-700 pt-8">
                                    <div>
                                        ...
                                        <p className="text-sm text-gray-500 dark:text-gray-400">{t('landing.stats_farmers')}</p>
                                    </div>
                                    <div>
                                        <p className="text-3xl font-bold text-gray-900 dark:text-white">₹5Cr</p>
                                        <p className="text-sm text-gray-500 dark:text-gray-400">{t('landing.stats_transactions')}</p>
                                    </div>
                                    <div>
                                        <p className="text-3xl font-bold text-gray-900 dark:text-white">50+</p>
                                        <p className="text-sm text-gray-500 dark:text-gray-400">{t('landing.stats_mandis')}</p>
                                    </div>
                                </div>
                            </div>
                                ...
                                <div className="relative rounded-2xl overflow-hidden shadow-2xl bg-gradient-to-br from-green-50 to-orange-50 dark:from-gray-800 dark:to-gray-900 p-8 border border-white/20 dark:border-gray-700">
                                    ...
                                    <div className="absolute -bottom-6 -left-6 bg-white dark:bg-card-dark p-4 rounded-xl shadow-xl border border-gray-100 dark:border-gray-600 z-20 max-w-xs animate-bounce" style={{ animationDuration: '3s' }}>
                                        <div className="flex items-center space-x-3 mb-2">
                                            ...
                                            <div>
                                                <p className="text-xs text-gray-500 dark:text-gray-400">{t('landing.earnings_today')}</p>
                ...
                <section className="py-20 bg-white dark:bg-card-dark transition-colors duration-300">
                    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                        <div className="text-center mb-16">
                            <h2 className="text-primary dark:text-green-400 font-semibold tracking-wide uppercase text-sm">{t('landing.why_choose_us')}</h2>
                            <p className="mt-2 text-3xl leading-8 font-extrabold tracking-tight text-gray-900 dark:text-white sm:text-4xl">{t('landing.everything_needs')}</p>
                        </div>
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
                            {[
                                { title: t('landing.feature_sell'), icon: 'storefront', color: 'green', desc: t('landing.feature_sell_desc') },
                                { title: t('landing.feature_mandi'), icon: 'analytics', color: 'orange', desc: t('landing.feature_mandi_desc') },
                                { title: t('landing.feature_secure'), icon: 'verified_user', color: 'blue', desc: t('landing.feature_secure_desc') }
                            ].map((feature, i) => (
                                <div key={i} className="bg-background-light dark:bg-background-dark rounded-2xl p-8 hover:shadow-lg transition duration-300 border border-transparent dark:border-gray-700 hover:border-primary/20">
